...
THE ESTIMATE YOU WROTE IN YOUR REPLY MUST BE IN THE RECORD — the same turn, not later. The commonest and worst version of failing to capture is subtle: they tell you a meal, you ESTIMATE its calories in your reply ("small goulash ~250, two sourdough ~260, running total ~510"), and you never call log. You did the whole job except the one part that matters — the number now lives only in the chat and is gone when the tab closes, and "Did you log it?" comes back "only when you asked." If you have written a calorie figure or a "today so far" for a meal, you have ALREADY estimated it, so it MUST have gone to log in that same turn. Never state a meal's calories, and never quote a running total, without log having been called for it first. The running total may only ever come from day_total, which log returns — a total you added up in prose is the tell that nothing was written. "Did you log it?" should always already be yes.
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        brief_hour:   { type: 'integer', description: 'The hour, 0-23 in THEIR timezone, when the nightly read lands as a notification and an email. Defaults to 20 (8pm). "Send it at nine" is 21. A notification at the wrong hour is how somebody mutes an app for good, so change it the moment they mention a time.' },
...
async function log(args, user) {
  ...
  const askedForDay = /where am i|where do i stand|totals?\b|how many|how much|so far|everything|break it down|my net|up or down|how('?s| is| am i doing)|\bburn(ed|t)?\b|my calories|calories today|what did i/i.test(text);
...
async function brief(args, user) {
  ...
  // A DAY STILL RUNNING IS NOT THE DAY THAT WAS CACHED. The 8pm verdict,
  // replayed at 10pm, quoted the steps and food as they stood at 8 — "my
  // steps are always behind by my night workout". For today, a cached verdict
  // is used only while the figures it was written from still stand.
  const stamp = f => JSON.stringify([f?.food?.calories, f?.food?.meals, f?.training?.sessions,
    f?.device?.steps, f?.device?.active_calories]);
  const stillTrue = date !== today || (cached?.facts && stamp(cached.facts) === stamp(facts));
  ...
  // or the steps.
  const wholeDay = verdict ? null : dayReadout({ day, balance, receipt, scored, week: facts.training_week, date, today, flags });
...
async function fullDayRead(userId, profile, date, { day: known = null } = {}) {
  ...
  const receipt = dayReceipt({ day, balance, date, today });
  // The care flags, off the thirty days already fetched: the whole-day read is
  // where "how much should I eat" gets answered, and under a flag no figure of
  // what is left is ever quoted.
  const flags = careFlags(range, profile, { openDate: today });
  return { day, balance, receipt, scored, week, flags,
           read: dayReadout({ day, balance, receipt, scored, week, date, today, flags }) };
}
...
Answer in 2-4 short sentences: what to do right now and why, in their units. If they have protein left to hit, name specific foods that would close it. If the window is shut, say how long until it opens rather than lecturing. Be ${profile.bluntness}. No lists, no headings, no disclaimers.` }],
  ...
  return {
    ...
    say: recommendation || [
      ...
      caloriesLeft != null && !flags.length ? (caloriesLeft < 0 ? `${Math.abs(caloriesLeft).toLocaleString('en-US')} kcal over target.` : `${caloriesLeft.toLocaleString('en-US')} kcal left.`) : null,
