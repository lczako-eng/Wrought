...
for(const file of ['workout.html','workout.js','workout.css','workout-clock.js']) assert.ok(sw.includes(`'/${file}'`));
const dashboard=readFileSync(new URL('../public/app.html',import.meta.url),'utf8');
assert.doesNotMatch(dashboard,/performance\.css|class="workout-launch"/, 'rejected dashboard redesign must not return');
assert.doesNotMatch(sw,/'\/performance\.css'/, 'removed visual stylesheet must not break shell installation');
