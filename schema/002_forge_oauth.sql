-- ============================================================================
-- FORGE — schema 002: OAuth 2.1
--
-- This is what makes "Sign in with Forge" appear inside ChatGPT and Claude
...
create table if not exists public.forge_oauth_clients (
  client_id     text primary key,
  client_name   text,
  redirect_uris text[] not null,
  grant_types   text[] not null default array['authorization_code','refresh_token'],
  scope         text not null default 'forge',
  created_at    timestamptz not null default now()
);

comment on table public.forge_oauth_clients is
...
create table if not exists public.forge_oauth_codes (
...
create index if not exists forge_oauth_codes_expiry_idx on public.forge_oauth_codes (expires_at);
...
create table if not exists public.forge_oauth_tokens (
...
create index if not exists forge_oauth_tokens_user_idx   on public.forge_oauth_tokens (user_id);
create index if not exists forge_oauth_tokens_expiry_idx on public.forge_oauth_tokens (expires_at);

create table if not exists public.forge_oauth_refresh (
  token_hash  text primary key,
  user_id     uuid not null references auth.users(id) on delete cascade,
  client_id   text,
  scope       text not null default 'forge',
...
create index if not exists forge_oauth_refresh_user_idx on public.forge_oauth_refresh (user_id);
...
alter table public.forge_oauth_clients enable row level security;
alter table public.forge_oauth_codes   enable row level security;
alter table public.forge_oauth_tokens  enable row level security;
alter table public.forge_oauth_refresh enable row level security;
...
create or replace function public.forge_oauth_gc()
returns void language sql security definer set search_path = public as $$
  delete from public.forge_oauth_codes  where expires_at < now() - interval '1 day';
  delete from public.forge_oauth_tokens where expires_at < now() - interval '7 days';
  delete from public.forge_oauth_refresh where expires_at < now() - interval '30 days';
$$;
