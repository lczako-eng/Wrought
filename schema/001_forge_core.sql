...
--
-- FORGE is the memory that survives the tab closing, plus an honest read on
-- what it means. The AI is only the interface. This schema is the product.
--
-- Design decisions worth defending later:
--
--   * ONE log table (forge_events) for everything a human says, and ONE
--     time-series table (forge_metrics) for everything a device pushes.
...
-- Everything except timezone is nullable on purpose: FORGE works knowing
-- nothing but what clock you're on, and must never hold a brief hostage
-- waiting for a birth year.

create table if not exists public.forge_profile (
...
do $$
begin
  if not exists (select 1 from pg_constraint where conname = 'forge_profile_units_valid') then
    alter table public.forge_profile add constraint forge_profile_units_valid
      check (units in ('metric', 'imperial'));
  end if;
  if not exists (select 1 from pg_constraint where conname = 'forge_profile_bluntness_valid') then
    alter table public.forge_profile add constraint forge_profile_bluntness_valid
...
comment on table public.forge_events is
  'Everything the user tells FORGE, one row per thing. The diary.';
comment on column public.forge_events.event_type is
  'food | drink | workout | weight | measurement | sleep | symptom | mood | supplement | note';
comment on column public.forge_events.local_date is
  'The calendar day in the user''s own timezone. Every brief groups on this and never on UTC.';
comment on column public.forge_events.detail is
...
comment on column public.forge_events.raw_input is
...
do $$
begin
  if not exists (select 1 from pg_constraint where conname = 'forge_events_type_valid') then
    alter table public.forge_events add constraint forge_events_type_valid
      check (event_type in ('food','drink','workout','weight','measurement',
                            'sleep','symptom','mood','supplement','note'));
  end if;
end $$;

create index if not exists forge_events_user_day_idx
  on public.forge_events (user_id, local_date desc);
create index if not exists forge_events_user_type_day_idx
  on public.forge_events (user_id, event_type, local_date desc);
create index if not exists forge_events_detail_idx
  on public.forge_events using gin (detail);

-- Idempotent ingest. Partial index so hand-logged rows (no ref) are unconstrained.
create unique index if not exists forge_events_source_ref_idx
  on public.forge_events (user_id, source, source_ref)
...
create table if not exists public.forge_metrics (
...
comment on table public.forge_metrics is
...
comment on column public.forge_metrics.source is
  'apple_health | oura | whoop | fitbit | garmin | withings | manual';

create index if not exists forge_metrics_user_metric_idx
  on public.forge_metrics (user_id, metric, measured_at desc);
create index if not exists forge_metrics_user_day_idx
  on public.forge_metrics (user_id, local_date desc);

-- The same night's sleep, sent four times, stays one row.
create unique index if not exists forge_metrics_dedupe_idx
  on public.forge_metrics (user_id, source, metric, measured_at);
...
create table if not exists public.forge_goals (
  id           uuid primary key default gen_random_uuid(),
  user_id      uuid not null references auth.users(id) on delete cascade,
...
create table if not exists public.forge_eating_window (
  user_id      uuid primary key references auth.users(id) on delete cascade,
...
comment on table public.forge_eating_window is
  'The hours the user has decided they eat between. FORGE never invents one and never imposes one — it only holds the line the user drew.';
...
-- /ingest with a key from forge_ingest_keys. Oura, Whoop, Fitbit, Garmin and
...
create table if not exists public.forge_connections (
  id               uuid primary key default gen_random_uuid(),
  user_id          uuid not null references auth.users(id) on delete cascade,
...
create unique index if not exists forge_connections_user_provider_idx
  on public.forge_connections (user_id, provider);
...
create table if not exists public.forge_ingest_keys (
  id           uuid primary key default gen_random_uuid(),
  user_id      uuid not null references auth.users(id) on delete cascade,
...
comment on table public.forge_ingest_keys is
...
create index if not exists forge_ingest_keys_user_idx
  on public.forge_ingest_keys (user_id) where not revoked;
...
create table if not exists public.forge_briefs (
  id           uuid primary key default gen_random_uuid(),
  user_id      uuid not null references auth.users(id) on delete cascade,
  local_date   date not null,
...
comment on table public.forge_briefs is
  'The daily read, kept. facts is arithmetic and is always trustworthy; verdict is written prose and is the only part of FORGE that is an opinion.';

create unique index if not exists forge_briefs_user_day_kind_idx
  on public.forge_briefs (user_id, local_date, kind);
...
create table if not exists public.forge_memory (
  id           uuid primary key default gen_random_uuid(),
  user_id      uuid not null references auth.users(id) on delete cascade,
  fact         text not null,
...
comment on table public.forge_memory is
...
create index if not exists forge_memory_user_idx
  on public.forge_memory (user_id, category) where active;
...
alter table public.forge_profile       enable row level security;
alter table public.forge_events        enable row level security;
alter table public.forge_metrics       enable row level security;
alter table public.forge_goals         enable row level security;
alter table public.forge_eating_window enable row level security;
alter table public.forge_briefs        enable row level security;
alter table public.forge_memory        enable row level security;
alter table public.forge_connections   enable row level security;
alter table public.forge_ingest_keys   enable row level security;

do $$
declare t text;
begin
  foreach t in array array['forge_profile','forge_events','forge_metrics','forge_goals',
                           'forge_eating_window','forge_briefs','forge_memory']
  loop
...
-- forge_connections and forge_ingest_keys are deliberately policy-free.
